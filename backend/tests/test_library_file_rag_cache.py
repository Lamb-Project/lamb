...
class TestFetchWithCache:
    ...
    def test_no_assistant_returns_explicit_error(self):
        from lamb.completions.rag import library_file_rag
        result = library_file_rag.rag_processor([], assistant=None)
        assert result["error"] == "no assistant configuration"


def test_transient_library_failure_is_retried_and_recovery_is_cached():
    from lamb.completions.rag import library_file_rag
    from lamb.completions import document_cache
    document_cache.clear()
    assistant = _make_assistant()
    failed = MagicMock(status_code=503)
    recovered = MagicMock(status_code=200, text="Recovered course material")
    with patch.object(library_file_rag.httpx, "get", side_effect=[failed, recovered]) as get:
        first = library_file_rag.rag_processor([], assistant=assistant)
        second = library_file_rag.rag_processor([], assistant=assistant)
        third = library_file_rag.rag_processor([], assistant=assistant)
    assert "HTTP 503" in first["error"]
    assert "error" not in second
    assert second["context"] == third["context"] == "Recovered course material"
    assert third["_timing"]["cache"] == "hit"
