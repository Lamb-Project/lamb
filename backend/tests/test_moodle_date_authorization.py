...
from lamb.moodle.analytics.authorization import validate_date_scope
...
RESPONSE = {'authorized':True,'courseid':7,'cmids':[42],'scopeonly':True,'basis':'stored_course_defaults','relative_dates':False,'dates':[]}


def test_exact_permission_only_call():
    client = SimpleNamespace(call=Mock(return_value=RESPONSE))
    validate_date_scope(client,SCOPE)
    client.call.assert_called_once_with(DEFAULT_DATES_FUNCTION,courseid=7,cmids=[42],scopeonly=1)


@pytest.mark.parametrize('response', [None,{},dict(RESPONSE,authorized=1),dict(RESPONSE,authorized=False),
    dict(RESPONSE,courseid=True),dict(RESPONSE,cmids=[43]),dict(RESPONSE,cmids=[42.0]),
    dict(RESPONSE,scopeonly=False),dict(RESPONSE,relative_dates=0),
    dict(RESPONSE,dates=[{'cmid':42,'due':123}]),dict(RESPONSE,extra='unexpected')])
def test_response_mismatch_denies(response):
    with pytest.raises(PermissionError):
        validate_date_scope(SimpleNamespace(call=lambda *a,**kw:response),SCOPE)
...
def test_bad_scope_does_not_call_source(scope):
    ...
    with pytest.raises(PermissionError): validate_date_scope(client,scope)
    client.call.assert_not_called()


@respx.mock
def test_saved_chart_and_listing_revoke_without_date_recollection(stores,tmp_path):
    ...
    identity=chartstore.save({'title':'Grades','course_id':7,'course_name':'Course',
        'as_of':'2026-09-22T12:00:00Z','timezone':'UTC','coverage':{},'rows':[], 'date_scopes':[SCOPE]},
        dict(rt.result_binding(),course_id=7,date_scopes=[SCOPE]), command='moodle.analytics.run')
    ...
    with patch('lamb.moodle.runtime.MoodleScope'):
        ...
        assert chartstore.listing()['items'][0]['date_scopes'] == [SCOPE]
    ...
    assert all(b'local_lambanalytics_default_dates' in call.request.content for call in route.calls)


@pytest.mark.parametrize('courses', [None,8])
def test_date_scope_requires_matching_bound_course(stores,courses):
    rt=runtime(stores)
    binding=dict(rt.result_binding(),date_scopes=[SCOPE])
...
@pytest.mark.parametrize('command,data', [
    ('moodle analytics result 00000000-0000-0000-0000-000000000001', {'course_id':7,'date_scopes':[SCOPE]}),
    ('moodle chart list',{'items':[{'course_id':7,'date_scopes':[SCOPE]}]}),
])
def test_liteshell_preserves_date_scope(stores,command,data):
    import asyncio
    ...
    assert result.result_binding['date_scopes'] == [SCOPE]
