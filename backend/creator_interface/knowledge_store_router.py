...
async def list_content(
    ks_id: str,
    auth: AuthContext = Depends(get_auth_context),
):  # pragma: no cover - shadowed by list_ks_content (same path, registered first)
    """List linked library items for a Knowledge Store.

    NOTE: this path is already served by ``list_ks_content`` above, which is
    registered first and therefore wins the route match. Kept for reference;
