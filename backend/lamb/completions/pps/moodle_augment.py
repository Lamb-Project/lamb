...
This prompt processor extends simple_augment to automatically inject the Moodle user
identifier into prompts. It adds support for the {moodle_user} template variable.

Usage:
1. Set an assistant's prompt_processor to "moodle_augment" in metadata
2. Use {moodle_user} in the prompt template to inject the user identifier

Example prompt template:
    You are a helpful academic assistant.
    
    The current user is: {moodle_user}
...
1. X-OpenWebUI-User-Email header - OpenWebUI user email (injected via __openwebui_headers__)
2. X-OpenWebUI-User-Id header - OpenWebUI user ID
...
8. "unknown" - Fallback if no user identification is available
"""
...
def _has_vision_capability(assistant: Assistant) -> bool:
    ...
    metadata_str = getattr(assistant, 'metadata', None) or getattr(
        assistant, 'api_callback', None)
    if not metadata_str:
        return False
...
def _extract_user_id(request: Dict[str, Any]) -> str:
    ...
    logger.info(
        "No user ID found in request (checked OpenWebUI headers and metadata), using 'unknown'")
    return "unknown"
...
def prompt_processor(
    ...
    - {moodle_user}: Injects the user's Moodle identifier
    ...
    if assistant:
        ...
        if assistant.prompt_template:
            ...
            uses_moodle = "{moodle_user}" in assistant.prompt_template

            # Get Moodle user ID if needed
            moodle_user = ""
            if uses_moodle:
                moodle_user = _extract_user_id(request)
                Timelog(f"moodle_augment: Moodle user: {moodle_user}", 2)
            ...
            if isinstance(last_message, list) and has_vision:
                ...
                # Add Moodle user if template uses it
                if uses_moodle:
