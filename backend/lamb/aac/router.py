...
def _resolve_agent_llm(user_email: str):
    """Allocate the same effective destination disclosed by the Moodle page."""
    from lamb.aac.driver import resolve_driver, public_driver
    target = resolve_driver(OrganizationConfigResolver(user_email))
    client = AsyncOpenAI(api_key=target['api_key'], base_url=target['base_url'])
    # Keep the resolved identity with this exact client, without resolving twice.
    client._lamb_aac_driver = public_driver(target)
    return client, target['model']
