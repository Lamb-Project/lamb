...
def remember_files(files, params, context):
    ...
    for item in files:
        ...
        if not item.get('isdir') and item.get('url'):
            identity=json.dumps([context['course_id'], params, item['url']],sort_keys=True)
...
def download_file(base_url, token, item, *, single_file):
    ...
    allowed={'.txt','.md','.json'} if single_file else {'.txt','.md','.json','.pdf'}
    if suffix not in allowed:
        raise ValueError('Use UTF-8 txt/md/json for single-file grounding; KB import also accepts PDF')
    ...
    try:
        with httpx.Client(timeout=httpx.Timeout(60,connect=10),follow_redirects=False) as client:
            with client.stream('GET',endpoint,params={'token':token}) as response:
