...
SELF_READS = frozenset({'site.info','site.functions','user.me','enrol.my-courses',
                       'grade.overview','message.list','message.conversations','message.unread','content.types'})


class MoodleRuntime:
    def __init__(self, store, *, cipher=None, cache_root=None):
        self.store=store
        self._cipher=cipher
    ...
    def available(self):
        try:
            ...
            return set()
        return {'moodle.'+key for key in SELF_READS} | {'moodle.sync','moodle.cache.show'}
    ...
    def execute(self, key, params):
        ...
        if key not in SELF_READS | {'sync','cache.show'}:
...
def attach_to_agent(agent, store):
    ...
    if facts:
        ...
        references += ['moodle sync COURSE_ID [--section course|forums|assignments|enrolment|calendar]',
