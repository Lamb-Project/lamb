...
from lamb._vendor.tau.tau_agent.tools import AgentTool, ToolCall
...
from lamb._vendor.tau.tau_ai.http import create_async_client
from lamb._vendor.tau.tau_ai.http_errors import provider_http_error_message
from lamb._vendor.tau.tau_ai.provider import CancellationToken
from lamb._vendor.tau.tau_ai.retry import provider_retry_event, retry_delay_seconds, wait_for_retry
...
class _ChatStreamParser:
    ...
    def __init__(self) -> None:
        ...
        self._usage: TokenUsage | None = None  # LAMB addition
    ...
    def feed(self, event: str) -> tuple[list[ProviderEvent], bool]:
        ...
        # LAMB addition: with stream_options.include_usage the final usage
        # chunk arrives with an empty choices array — capture it before the
        # first-choice early return below would drop it.
        usage = chunk.get("usage")
        if isinstance(usage, Mapping):
            self._usage = _token_usage_from_chat(usage) or self._usage
    ...
    def finalize(self) -> list[ProviderEvent]:
        ...
        events.append(
            ProviderResponseEndEvent(
                ...
                usage=self._usage,  # LAMB addition
            )
        )
        return events
...
class _ResponsesStreamParser:
    ...
    def feed(self, event: str) -> tuple[list[ProviderEvent], bool]:
        ...
        if chunk_type in ("response.output_text.delta", "response.refusal.delta"):
            ...
            # LAMB addition: the Responses API reports usage on the terminal
            # event as response.usage {input_tokens, output_tokens, ...}.
            response = chunk.get("response")
            if isinstance(response, Mapping):
                usage = response.get("usage")
                if isinstance(usage, Mapping):
                    self._usage = _token_usage_from_responses(usage) or self._usage
            return [], True
...
# LAMB addition: token-usage extraction for both wire dialects. Tolerant of
# absent or malformed fields — usage is telemetry, never a failure source.

def _token_usage_from_chat(usage: Mapping[str, Any]) -> TokenUsage | None:
    """Chat Completions dialect: prompt_tokens / completion_tokens."""
    try:
        return TokenUsage(
...
def _token_usage_from_responses(usage: Mapping[str, Any]) -> TokenUsage | None:
    """Responses API dialect: input_tokens / output_tokens."""
    try:
