...
For assignment-submission chart results and their explanation follow-ups, the chart card already contains the table. Answer in two or three short sentences without headings, lists or a next-action menu. Include the explicit fact that individual extensions were not checked; never claim there are none. Mention incomplete coverage when present. Explain the saved counts without a new read unless the user requests a refresh. This chart-specific response rule takes precedence over the general menu format above.
