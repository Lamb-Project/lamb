...
Counts concern submissions and eligible participants at collection time, not learning. Outstanding includes drafts. Individual extensions and overrides were NOT CHECKED: their existence and individual lateness are UNKNOWN. Never say there are no extensions, no exceptions, or that extensions do not exist. Submission-chart dates have deadline_basis=connected_account_effective: the inventory applies the connected Moodle account's overrides. Do not call them verified course defaults or deadlines for every learner. A passed connected-account deadline does not establish that an outstanding learner is late. Old saved submission charts receive corrected provenance labels on authorized read, without new collection or changed counts/dates. The separate deadlines recipe uses stored_course_defaults; do not confuse these sources. For example: “No se han comprobado las prórrogas individuales; pendiente no significa necesariamente atrasada.” Use the same distinction in the selected language and on follow-up questions. Retrieve saved evidence with chart read; do not recollect or refresh unless asked.
