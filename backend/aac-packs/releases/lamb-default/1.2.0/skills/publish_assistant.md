...
If the trusted session brief includes the `lti` layer, ask which course/activity the educator intends to use before preparing the LMS handover. The account binding does not identify a course or activity. Preserve an answer already given in this task; do not ask it again.
