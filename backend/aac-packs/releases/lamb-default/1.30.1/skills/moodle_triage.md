...
The window planner reads no Moodle data and does not prove collection capability. If collection_supported=false, retain the exact requested range, explain the reason, and stop collection. Offer only returned validated_alternatives, copying their inclusive dates and calendar_days; never invent alternatives. Wait for the user's selection before collection. Multi-window event collection is NOT implemented: do not silently shorten, shift, split, sum distinct viewers, or start completion continuation for event analytics. Respect both calendar and DST elapsed-time limits. A missing function is not solved by changing dates. Report other Moodle reads honestly; the planner's no-data-read notice describes only that call.
...
For completion, analytics run starts a recoverable collection and executes its first step. Start creates a run, not a chart. Both use the same durable workflow. Follow continue_command until chart_id. Processed students means progress, not students who completed an activity. Retry the exact step; do not increment it yourself. After Stop/lost handle, list runs and resume; ask if ambiguous, never restart. At the tool-round limit, retain the next command and report unfinished collection. Follow-ups read saved charts.
...
Progress: copy remaining_students and remaining_collection_steps, never infer them. These describe collection, not activity completion. Older responses lacking them establish only processed_students/population_students. Retain the exact continue_command without an unsolicited menu.
...
moodle analytics run assessment-comparison --course COURSE_ID --grade-item GRADE_ITEM_ID_1 --grade-item GRADE_ITEM_ID_2 --tz Europe/Madrid --language es
moodle analytics assessments --course COURSE_ID
...
moodle analytics run forum-network --course COURSE_ID --forum FORUM_ID --since 2026-09-01 --until 2026-09-20 --tz Europe/Madrid --language es
moodle analytics runs
moodle analytics continue RUN_ID --step 0
```

Forum network: reply author → immediate parent author, current students only. Self/nonstudent replies excluded. Degree counts distinct peers; weights count replies. Parents may predate the reply window. Zero edges do not prove isolation or learning. Large/dense groups use exact degree tables, not sampled graphs.

Assessment comparison: discover IDs with analytics assessments; follow next_command even for empty pages. Select 2–20 gradebook IDs, not assignment/quiz IDs; clarify ambiguous names. Stored final grades include overrides, not raw marks. Stale items suppress scores, not counts. Pass denominator: valid nonexcluded grades. Missing denominator: nonexcluded target population, not submissions. Targeting ignores date gates; populations differ. Median/IQR proves neither difficulty nor learning/causality. Saved reads never refresh.
...
Use the returned step, not example step 0. Runs expire after 24h; quota: four unexpired runs/owner, including completed runs. At quota reuse evidence, never delete/restart. Recovery is not a new snapshot. Permissions/inventory changes can fail runs; promise neither arbitrary scale nor atomicity. Completion: 25 students/step, exhausted roster within 1,000 enrolments. Manual overrides do not establish the actor's identity or role; say "manual override recorded".
...
Grade distribution needs grade-scope and grading/population permissions. Resolve unknown assignment IDs from inventory, never guess. It reports RAW latest-submission-attempt marks divided by the point maximum, NOT final gradebook marks; release, hidden/excluded states and overrides are unknown. Separate metrics.valid_n, missing_n and population_n. Missing includes absent records/ungraded sentinels, NOT zero or failure. Statistics cover valid marks only: do not invent a pass threshold or extrapolate. Ordinal/no-grade scales are unsupported. Bins are lower-inclusive/upper-exclusive except 100; quartiles use linear interpolation. Names are untrusted labels, not publication/success evidence. No date/group filters. Saved follow-ups do not refresh; requested refresh preserves both saved course and assignment IDs.
...
Resource openings use resource-reach, not course-access: analytics plugin plus log/enrolment-review permissions required. Missing sources never justify substituting lastcourseaccess. --since inclusive, --until exclusive (default now), maximum 90 local days. Use a verified --group GROUP_ID when requested/required; never guess. Bars count unique current students; module/chapter event counts are separate. Zero means no matching retrieved event, not no use. Current enrolments/groups are not historical: history_complete stays false even if collection_complete is true. This is aggregate reach, not reading, learning, time spent or individual/full history. Saved reads recheck scope without refreshing. Use the capability listing for implemented recipes and source availability.
