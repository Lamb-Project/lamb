...
test.describe.serial("Knowledge Base Detail Modals", () => {
  ...
  test("Create a test KB with a file for modal tests", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /knowledge base details/i }),
  ...
  test("Delete file modal shows when clicking Delete button", async ({
    page,
  }) => {
    await page.goto("knowledgebases");
    await page.waitForLoadState("networkidle");
    ...
    await expect(modal.locator("h2, h3")).toContainText(/delete/i);
  ...
  test("Cancel button closes delete file modal without deleting", async ({
    page,
  }) => {
    await page.goto("knowledgebases");
    await page.waitForLoadState("networkidle");
  ...
  test("Confirm button deletes the file", async ({ page }) => {
    ...
    await expect(page.getByText("ikasiker_fixture.txt")).not.toBeVisible({
      timeout: 5_000,
    });
  });
