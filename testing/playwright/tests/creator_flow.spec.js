...
test.describe.serial("Creator flow (assistants + chat)", () => {
  ...
  test("Assistant chat responds with expected answer", async ({
    page,
  }, testInfo) => {
    ...
    if (!ASSISTANT_ID) {
      testInfo.skip(
        true,
        "ASSISTANT_ID not set in tests/.env - skipping chat test",
      );
      return;
    }
    ...
    if (!chatTabExists) {
      ...
      if (!chatTabAltExists) {
        testInfo.skip(
          true,
          `Assistant ID ${ASSISTANT_ID} not found or doesn't have chat enabled - skipping`,
        );
        return;
      }
