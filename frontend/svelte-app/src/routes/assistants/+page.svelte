<script>
	...
	$effect(() => {
		...
		if (
			...
			if (rubricFetchTriggered) {
				// Reset rubric fetch trigger as well
...
<div class="mb-6 border-b border-gray-200">
	<nav class="-mb-px flex space-x-4" aria-label="Tabs">
		...
		<button
			class="inline-flex items-center gap-1.5 rounded-lg px-4 py-2.5 text-sm font-medium whitespace-nowrap transition-all duration-150 {currentView ===
			'create'
		...
		{#if $user.owiUrl}
			<a
				...
				class="inline-flex items-center gap-1 rounded-t-md border-b-2 border-transparent px-1 py-4 text-sm font-medium whitespace-nowrap text-gray-800 transition-colors duration-150 hover:border-gray-400 hover:text-gray-900"
			>
...
{#if currentView === 'list'}
	<div class="mt-6">
		<div class="rounded-lg border border-gray-200 bg-white p-4 shadow">
			<AssistantsList on:delete={handleDeleteRequest} on:export={handleExportRequest} />
		</div>
	</div>
...
{:else if currentView === 'detail'}
	...
	<div class="mb-4 flex space-x-4 border-b border-gray-300">
		...
		{#if canShare && (isOwner || canManageSharing)}
			<button
				class="rounded-t-md px-4 py-2 text-sm font-medium {detailSubView === 'share'
					? 'text-brand border border-b-0 border-gray-300 bg-gray-100'
					: 'text-gray-600 hover:text-gray-800'}"
		...
		{#if canViewAnalytics}
			<button
				class="rounded-t-md px-4 py-2 text-sm font-medium {detailSubView === 'analytics'
					? 'text-brand border border-b-0 border-gray-300 bg-gray-100'
					: 'text-gray-600 hover:text-gray-800'}"
		...
		{#if isOwner}
			<button
				class="rounded-t-md px-4 py-2 text-sm font-medium {detailSubView === 'tests'
					? 'text-brand border border-b-0 border-gray-300 bg-gray-100'
					: 'text-gray-600 hover:text-gray-800'}"
		...
		{#each aacTabs.filter((t) => t.assistantId === selectedAssistantData?.id) as tab}
			<button
				class="group flex items-center gap-1 rounded-t-md px-4 py-2 text-sm font-medium {detailSubView ===
					'aac' && activeAacSessionId === tab.id
					? 'text-brand border border-b-0 border-gray-300 bg-gray-100'
					: 'text-gray-600 hover:text-gray-800'}"
			...
			>
				...
				<span
					...
					onclick={(e) => {
						e.stopPropagation();
						closeTab(tab.id);
						if (activeAacSessionId === tab.id) detailSubView = 'properties';
					}}
					onkeydown={(e) => {
						if (e.key === 'Enter') {
							e.stopPropagation();
					...
					class="ml-1 cursor-pointer text-xs opacity-0 group-hover:opacity-60 hover:opacity-100"
					title="Close session">✕</span
				>
			</button>
		{/each}
		...
		{#if isOwner}
			<span class="mx-2 h-6 self-center border-l border-gray-300"></span>
			<button
				onclick={() => launchAacSkill('explain-assistant')}
				disabled={aacLaunching}
				class="rounded-md border border-purple-200 bg-purple-50 px-3 py-1.5 text-xs font-medium text-purple-700 hover:bg-purple-100 disabled:cursor-wait disabled:opacity-50"
			...
			<button
				...
				disabled={aacLaunching}
				class="rounded-md border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-medium text-emerald-700 hover:bg-emerald-100 disabled:cursor-wait disabled:opacity-50"
			...
			{#if detailSubView === 'properties'}
				...
				{#key selectedAssistantData?.id}
					...
					{#if !isOwner && accessLevel === 'read_only'}
						<div class="flex items-center gap-2 border-b border-amber-200 bg-amber-50 px-6 py-2">
							<svg
							...
							<span class="text-sm text-amber-700">
								{currentLocale
									? $_('assistants.detail.readOnlyBanner', {
											default: 'Read-only view — This assistant belongs to {owner}',
											values: { owner: selectedAssistantData?.owner || '' }
										})
									: `Read-only view — This assistant belongs to ${selectedAssistantData?.owner || ''}`}
							</span>
						</div>
					{/if}
							...
							{#if isOwner}
								...
								>
									{#if isPublishing}
										...
										>
										{currentLocale
											? $_('common.updating', { default: 'Updating...' })
											: 'Updating...'}
					...
					<div class="flex flex-col px-6 py-4 md:flex-row md:space-x-6">
						...
						<div class="flex-grow space-y-4 md:w-2/3">
							...
							<div>
								...
								<div
									class="mt-1 block w-full cursor-not-allowed rounded-md border border-gray-300 bg-gray-100 px-3 py-2 text-gray-900 shadow-sm sm:text-sm"
								>
									{selectedAssistantData.name?.replace(/^\d+_/, '') ||
							...
							<div>
								...
								<div
									class="mt-1 block min-h-[6em] w-full cursor-text rounded-md border border-gray-300 bg-gray-100 px-3 py-2 whitespace-pre-wrap text-gray-900 shadow-sm select-text sm:text-sm"
								>
							...
							<div>
								...
								<div
									class="mt-1 block min-h-[8em] w-full cursor-text rounded-md border border-gray-300 bg-gray-100 px-3 py-2 whitespace-pre-wrap text-gray-900 shadow-sm select-text sm:text-sm"
								>
							...
							<div>
								...
								<div
									class="mt-1 block max-h-96 w-full cursor-text overflow-y-auto rounded-md border border-gray-300 bg-gray-100 px-4 py-3 whitespace-pre-wrap text-gray-900 shadow-sm select-text sm:text-sm"
								>
							...
							{#if selectedAssistantData}
								...
								{#if apiCallback.rag_processor === 'rubric_rag'}
									<div class="mt-6 border-t border-gray-200 pt-6">
										...
										{#if loadingRubric}
											<div
												class="rounded-md border border-gray-200 bg-white p-3 text-sm text-gray-500 italic"
											>
										...
										{:else if rubricError}
											...
											>
												<strong
													>{$_('assistants.form.rubric.error', {
														default: 'Error loading rubric:'
													})}</strong
												>
												{rubricError}
											</div>
										{:else if rubricMarkdown}
											<div class="rounded-md border border-gray-300 bg-white p-4">
													...
													<div class="space-y-1 text-xs text-gray-600">
														<div>
															<span class="font-medium"
																>{$_('assistants.form.rubric.id', { default: 'Rubric ID:' })}</span
															>
															<code class="ml-1 rounded bg-gray-100 px-1 py-0.5 text-xs"
																>{apiCallback.rubric_id || 'N/A'}</code
															>
														</div>
														<div>
															<span class="font-medium"
																>{$_('assistants.form.rubric.format.label', {
																	default: 'Format:'
																})}</span
															>
															<span
																class="ml-1 inline-flex items-center rounded bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-800"
															>
												...
												<details open>
													<summary
														class="mb-2 cursor-pointer text-sm font-medium text-blue-600 hover:text-blue-800"
													>
														{$_('assistants.form.rubric.viewRubric', {
															default: 'View Rubric Content'
														})}
													</summary>
													<div
														class="mt-3 max-h-96 overflow-y-auto rounded border border-gray-200 bg-gray-50 p-4"
													>
														<pre
															class="font-mono text-xs whitespace-pre-wrap text-gray-800">{rubricMarkdown}</pre>
													</div>
												</details>
											</div>
										{:else}
									...
									<div class="rounded border border-blue-200 bg-blue-50 p-4">
										...
										<div class="space-y-1 text-sm break-words text-blue-700">
											<div>
												<span class="font-medium"
													>{currentLocale
														? $_('assistants.detail.ltiAssistantName', {
																default: 'Assistant Name'
															})
														: 'Assistant Name'}:</span
												>
												<code class="ml-1 rounded bg-blue-100 px-1"
													>{selectedAssistantData.name}</code
												>
											</div>
											<div>
												<span class="font-medium"
													>{currentLocale
														? $_('assistants.detail.ltiModelId', { default: 'Model ID' })
														: 'Model ID'}:</span
												>
												<code class="ml-1 rounded bg-blue-100 px-1">{selectedAssistantData.id}</code
												>
											</div>
											<div>
												<span class="font-medium"
									...
									{#if selectedAssistantData}
										...
										<div class="space-y-3 text-sm">
											...
											<div>
												<div class="mb-1 font-medium text-gray-700">
													{currentLocale
														? $_('assistants.form.vision.label', { default: 'Vision Capability' })
														: 'Vision Capability'}
												</div>
												<div class="rounded border border-gray-200 bg-white p-2">
													{#if apiCallback.capabilities?.vision}
														<span class="inline-flex items-center text-green-800">
															...
															{currentLocale
																? $_('assistants.form.vision.enabled', { default: 'Enabled' })
																: 'Enabled'}
														</span>
													{:else}
														<span class="text-gray-500">
															{currentLocale
																? $_('assistants.form.vision.disabled', { default: 'Disabled' })
																: 'Disabled'}
														</span>
													{/if}
												</div>
											...
											<div>
												<div class="mb-1 font-medium text-gray-700">Image Generation</div>
												<div class="rounded border border-gray-200 bg-white p-2">
															...
															{:else if selectedAssistantData.RAG_collections}
																<div class="space-y-1 rounded border border-gray-200 bg-white p-2">
																	{#each selectedAssistantData.RAG_collections.split(',') as kbId}
																		{@const kb = accessibleKnowledgeBases.find(
																			(k) => k.id === kbId
																		)}
																		<span
					...
					{#if selectedAssistantData}
						<div class="mt-6 border-t border-gray-200 px-6 py-4">
							<div class="flex flex-col text-sm text-gray-600 sm:flex-row sm:space-x-8">
								<div>
									<span class="font-medium text-gray-700"
										>{currentLocale
											? $_('assistants.detail.created', { default: 'Created:' })
											: 'Created:'}</span
									>
								...
								<div>
									<span class="font-medium text-gray-700"
										>{currentLocale
											? $_('assistants.detail.updated', { default: 'Updated:' })
											: 'Updated:'}</span
									>
				...
				<div class="px-6 py-6">
					<div class="mb-4">
						<h2 class="mb-2 text-xl font-semibold text-gray-800">
							{currentLocale
								? $_('assistants.sharing.title', { default: 'Shared Users' })
								: 'Shared Users'}
						</h2>
						<p class="mb-4 text-sm text-gray-600">
							{currentLocale
								? $_('assistants.sharing.description', {
										default: 'Manage who has access to this assistant'
									})
					...
					{:else if sharedUsers.length === 0}
						<div class="rounded-lg border border-gray-200 bg-gray-50 p-6 text-center">
							<p class="text-gray-600">
								{currentLocale
									? $_('assistants.sharing.noShares', {
											default: 'This assistant is not shared with anyone yet.'
										})
									: 'This assistant is not shared with anyone yet.'}
							</p>
						</div>
					{:else}
					...
					<div class="mt-6">
						...
						>
							{currentLocale
								? $_('assistants.sharing.manageButton', { default: 'Manage Shared Users' })
								: 'Manage Shared Users'}
						</button>
					</div>
				</div>
...
<ConfirmationModal
	...
	message={currentLocale
		? $_('assistants.deleteModal.confirmation', {
				values: { name: assistantToDeleteName || '' },
				default: `Are you sure you want to delete the assistant "${assistantToDeleteName}"? This action cannot be undone.`
			})
		: `Are you sure you want to delete the assistant "${assistantToDeleteName}"? This action cannot be undone.`}
	confirmText={currentLocale
