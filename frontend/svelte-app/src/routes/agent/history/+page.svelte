...
<div class="mx-auto max-w-6xl px-4 py-6">
	...
	<div class="mb-4 flex flex-wrap gap-3">
		<div class="flex overflow-hidden rounded-md border border-gray-300 text-sm">
			{#each [['all', 'All'], ['today', 'Today'], ['active', 'Active'], ['errors', 'Errors']] as [val, label]}
				<button
					onclick={() => (filter = val)}
					class="px-3 py-1.5 {filter === val
						? 'bg-brand text-white'
						: 'bg-white text-gray-700 hover:bg-gray-50'}">{label}</button
				>
			{/each}
		</div>
		<select bind:value={skillFilter} class="rounded-md border border-gray-300 px-2 py-1.5 text-sm">
	...
	<!-- Sessions table -->
	{#if loading}
		<div class="py-12 text-center text-gray-400">Loading sessions...</div>
	{:else}
		{@const filtered = filteredSessions()}
		<div class="mb-2 text-sm text-gray-500">{filtered.length} sessions</div>
		<div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow">
			<table class="w-full text-sm">
				<thead>
					<tr class="border-b bg-gray-50 text-left text-xs tracking-wider text-gray-500 uppercase">
				...
				<tbody>
					{#each filtered as s}
						<tr class="border-b border-gray-100 hover:bg-gray-50">
							...
							<td
								class="px-4 py-3 text-center {(s.tool_errors || 0) > 0
									? 'font-semibold text-red-600'
									: 'text-gray-400'}">{s.tool_errors || 0}</td
							>
							<td class="px-4 py-3 text-xs text-gray-500">{formatDate(s.updated_at)}</td>
							<td class="px-4 py-3">
								<div class="flex gap-2">
									{#if s.status === 'active'}
										<button
											onclick={() => resumeSession(s)}
											class="text-xs font-medium text-blue-600 hover:text-blue-800">Resume</button
										>
									{/if}
									<a href="/agent/history/{s.id}" class="text-xs text-gray-600 hover:text-gray-800"
										>Review</a
									>
									<button
					...
					{#if filtered.length === 0}
						<tr
							><td colspan="8" class="px-4 py-8 text-center text-gray-400 italic"
								>No sessions match your filters.</td
							></tr
						>
					{/if}
				</tbody>
