	...
	async function handleCreateOrganization(e) {
		...
		if (!slug || !name) {
			createOrgError = localeLoaded
				? $_('admin.organizations.errors.fillRequired', {
		...
		if (!/^[a-z0-9-]+$/.test(slug)) {
			createOrgError = localeLoaded
				? $_('admin.organizations.errors.slugInvalid', {
						default: 'Slug must contain only lowercase letters, numbers, and hyphens.'
					})
		...
		if (signupEnabled && (!signupKey || signupKey.trim().length < 8)) {
			createOrgError = localeLoaded
				? $_('admin.organizations.errors.signupKeyLength', {
						default: 'Signup key must be at least 8 characters long when signup is enabled.'
					})
				: 'Signup key must be at least 8 characters long when signup is enabled.';
			return;
		}
		...
		if (signupKey && !/^[a-zA-Z0-9_-]+$/.test(signupKey)) {
			createOrgError = localeLoaded
				? $_('admin.organizations.errors.signupKeyInvalid', {
						default: 'Signup key can only contain letters, numbers, hyphens, and underscores.'
					})
				: 'Signup key can only contain letters, numbers, hyphens, and underscores.';
			return;
		}
		...
		try {
			...
			if (!token) {
				throw new Error(
					localeLoaded
						? $_('admin.organizations.errors.authTokenNotFound', {
			...
			const payload = {
				...
				name: name,
				admin_user_id:
					adminUserId && adminUserId !== 'null' && adminUserId !== ''
			...
			if (response.data) {
				...
				throw new Error(
					localeLoaded
						? $_('admin.organizations.errors.createFailed', {
			...
			if (axios.isAxiosError(err) && err.response?.data?.detail) {
				...
				createOrgError = localeLoaded
					? $_('admin.organizations.errors.unknown', {
							default: 'An unknown error occurred while creating the organization.'
						})
				...
				{:else}
					<form class="mt-4" onsubmit={handleCreateOrganization}>
						...
						<div class="mb-4 text-left">
							...
							{/if}
							<p class="mt-1 text-xs text-gray-500 italic">
								Optionally select a user from the system organization to become admin. You can also
								create the organization without an admin and promote a member later from the
							...
							<div id="signup_config_section" class="mb-3">
								<label class="flex items-center">
									<input
										type="checkbox"
										name="signup_enabled"
										bind:checked={newOrg.signup_enabled}
										class="mr-2"
									/>
						...
						<div class="mb-4 text-left">
							<label class="flex items-center">
								<input
									type="checkbox"
									name="use_system_baseline"
									bind:checked={newOrg.use_system_baseline}
									class="mr-2"
								/>
