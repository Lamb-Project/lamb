			...
			<div class="flex">
				...
				<div class="flex flex-shrink-0 items-center">
					<div class="flex items-center space-x-2">
						...
						<div class="text-lg font-bold">
							...
							>
							<span
								class="rounded bg-gray-200 px-1 py-0.5 text-xs"
								title="Version: {VERSION_INFO.version}, Commit: {VERSION_INFO.commit}, Branch: {VERSION_INFO.branch}"
								>{versionDisplay}</span
							>
						</div>
					</div>
				...
				<div class="hidden sm:ml-4 sm:flex sm:items-center sm:gap-1">
					...
					<a
						...
						class="inline-flex items-center border-b-2 px-2 pt-1 text-sm font-medium whitespace-nowrap {$page
							.url.pathname ===
						base + '/assistants'
					...
					<a
						href="{base}/agent"
						class="inline-flex items-center border-b-2 px-2 pt-1 text-sm font-medium whitespace-nowrap {$page.url.pathname.startsWith(
							base + '/agent'
						)
							? 'border-[#2271b3] text-gray-900'
							: 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700'} {!$user.isLoggedIn
					...
					{#if $user.isLoggedIn && $user.data?.organization_role === 'admin' && $user.data?.role !== 'admin'}
						<!-- Org Admin link - only for org admins (not system admins) -->
						<a
							href="{base}/org-admin"
							class="inline-flex items-center border-b-2 px-2 pt-1 text-sm font-medium whitespace-nowrap {$page
								.url.pathname ===
					...
					<div class="tools-menu relative flex h-full items-center">
						<button
							type="button"
						...
						>
							<span class="pointer-events-none"
								>{localeLoaded
									? $_('nav.sources', { default: 'Sources of Knowledge' })
									: 'Sources of Knowledge'}</span
							>
							<svg
								class="pointer-events-none ml-1 h-4 w-4 transition-transform duration-200 {toolsMenuOpen
									? 'rotate-180'
									: ''}"
								fill="none"
								stroke="currentColor"
						...
						{#if toolsMenuOpen}
							<div
								class="absolute top-full left-0 z-50 w-52 rounded-b-md border border-gray-200 bg-white shadow-lg"
							>
								<div class="py-2">
									<a
			...
			<div class="flex items-center gap-3">
				...
				<div class="flex flex-col items-end gap-0.5">
					{#if $user.isLoggedIn}
						<button
							onclick={logout}
							class="inline-flex items-center justify-center rounded bg-red-600 px-2 py-1 text-xs font-medium text-white hover:bg-red-700"
						>
