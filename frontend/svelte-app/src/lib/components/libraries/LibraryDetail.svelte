...
<script>
	...
	async function handleDeleteItemConfirm() {
		...
		try {
			...
			// FR-10: a 409 from /creator/libraries/.../items/... carries a structured
			// detail body { message, knowledge_stores: [{id, name}] }. Close the modal
			// so the user isn't trapped, and surface the message + KS names so they
			// know what they need to unlink before the delete can succeed.
			showDeleteItemModal = false;
			const status = err?.response?.status;
			const detail = err?.response?.data?.detail;
			if (status === 409 && detail && typeof detail === 'object') {
				const ksNames =
					Array.isArray(detail.knowledge_stores) && detail.knowledge_stores.length
						? detail.knowledge_stores
								.map(/** @param {any} k */ (k) => k?.name || k?.id || '?')
								.join(', ')
						: '';
				const msg = detail.message || 'This item is referenced by a Knowledge Store.';
				error = ksNames ? `${msg} (${ksNames})` : msg;
			} else {
