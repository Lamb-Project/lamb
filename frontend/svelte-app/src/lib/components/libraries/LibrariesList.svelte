	...
	{#if loading}
		<div class="p-6 text-center">
			<div class="animate-pulse text-gray-500">
				{$_('libraries.loading', { default: 'Loading libraries...' })}
			</div>
		</div>
	{:else if error}
				...
				<thead class="bg-gray-50">
					<tr>
						<th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase"
							>{$_('libraries.name', { default: 'Name' })}</th
						>
						<th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase"
							>{$_('libraries.items.title', { default: 'Items' })}</th
						>
						<th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase"
							>{$_('libraries.sharing.label', { default: 'Sharing' })}</th
						>
						<th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase"
							>{$_('libraries.createdAt', { default: 'Created' })}</th
						>
					...
					{#each displayLibraries as lib (lib.id)}
						<tr class="hover:bg-gray-50">
							<td class="px-4 py-3">
								...
								{#if lib.description}
									<p class="mt-0.5 max-w-xs truncate text-xs text-gray-500">{lib.description}</p>
								{/if}
							</td>
							...
							<td class="px-4 py-3">
								{#if currentTab === 'my'}
									<button
										...
										class="rounded border px-2 py-1 text-xs {lib.is_shared
