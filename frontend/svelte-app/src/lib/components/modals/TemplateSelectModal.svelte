<script>
	...
	// Props
	let { onSelect = (/** @type {any} */ template) => {} } = $props();
	...
	function handleTemplateClick(event) {
		...
		const template = [...filteredUserTemplates, ...filteredSharedTemplates].find(
			(t) => t.id === templateId
		);
		if (template) {
...
{#if $templateSelectModalOpen}
	...
	<div
		class="bg-opacity-75 fixed inset-0 z-50 flex items-center justify-center bg-gray-500"
		onclick={handleBackdropClick}
		role="button"
		tabindex="-1"
	>
		<div class="flex max-h-[80vh] w-full max-w-5xl flex-col rounded-lg bg-white shadow-xl">
			<!-- Header -->
			<div class="border-b border-gray-200 px-6 py-4">
				<div class="flex items-center justify-between">
					<h2 class="text-xl font-bold text-gray-900">
						{$locale
							? $_('promptTemplates.selectTemplate', { default: 'Select Prompt Template' })
				...
				<div class="mt-4">
					<input
						...
						placeholder={$locale
							? $_('promptTemplates.search', { default: 'Search templates...' })
							: 'Search templates...'}
			...
			<div class="flex border-b border-gray-200 px-6">
				<button
					...
					class="border-b-2 px-4 py-3 text-sm font-medium {$currentTab === 'my'
				...
				<button
					...
					class="border-b-2 px-4 py-3 text-sm font-medium {$currentTab === 'shared'
						? 'border-blue-500 text-blue-600'
						: 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700'}"
				>
					{$locale
				...
				{:else if $currentTab === 'my'}
					{#if filteredUserTemplates.length === 0}
						<div class="py-8 text-center text-gray-500">
							{searchQuery
								? $locale
									? $_('promptTemplates.noResults', { default: 'No templates found' })
									: 'No templates found'
								: $locale
									? $_('promptTemplates.noTemplates', { default: 'No templates yet' })
									: 'No templates yet'}
						</div>
					{:else}
						<div class="space-y-2">
							{#each filteredUserTemplates as template (template.id)}
								<button
									...
									class="w-full rounded-lg border-2 px-4 py-3 text-left transition-colors {selectedTemplate?.id ===
									template.id
								...
								>
									<div class="flex items-start justify-between">
										...
										{#if selectedTemplate?.id === template.id}
											<svg
												class="ml-2 h-5 w-5 flex-shrink-0 text-blue-500"
												fill="currentColor"
												viewBox="0 0 20 20"
											>
												<path
					...
					{/if}
				{:else if filteredSharedTemplates.length === 0}
					<div class="py-8 text-center text-gray-500">
						{searchQuery
							? $locale
								? $_('promptTemplates.noResults', { default: 'No templates found' })
								...
								<div class="flex items-start justify-between">
									<div class="flex-1">
										...
										<div class="mt-2 flex items-center space-x-2 text-xs text-gray-500">
											...
											{/if}
											<span
												>Updated: {new Date(template.updated_at * 1000).toLocaleDateString()}</span
											>
										</div>
									</div>
									{#if selectedTemplate?.id === template.id}
			...
			<div class="flex justify-end space-x-3 border-t border-gray-200 px-6 py-4">
				...
				>
					{$locale
						? $_('promptTemplates.applyTemplate', { default: 'Apply Template' })
						: 'Apply Template'}
				</button>
			</div>
		</div>
