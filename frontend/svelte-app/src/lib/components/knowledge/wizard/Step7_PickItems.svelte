...
<div class="space-y-4">
    ...
    <p class="text-sm text-gray-600">
        {$_('knowledge.wizard.step7.description', {
            default: 'These items will be ingested into the Knowledge Store. All ready items are selected by default.'
        })}
    </p>
    ...
    {#if isNewLibrary}
        <div class="rounded-md border border-blue-200 bg-blue-50 p-3 text-sm text-blue-800">
            {$_('knowledge.wizard.step7.newLibraryNote', {
                default: 'You are creating a new Library. Any files you added in the previous step will be uploaded and automatically queued for ingestion when you click "Create".'
            })}
        </div>
    ...
    {:else}
        ...
        <div class="max-h-72 overflow-y-auto border border-gray-200 rounded">
            {#each items as item (item.id)}
                <label class="flex items-center gap-3 px-3 py-2 border-b border-gray-100 hover:bg-gray-50 cursor-pointer">
                    <input
                        type="checkbox"
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="text-xs text-gray-400 truncate">{item.source_type} · {item.id}</div>
                    </div>
                </label>
            {/each}
        </div>
