...
<script>
    ...
    $effect(() => {
        const valid =
            !!chunkingStrategy && !!embeddingVendor && !!embeddingModel && !!vectorDb;
...
<div class="space-y-4">
    ...
    <div class="rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800" role="note">
        ...
        <p class="mt-1 text-xs">
            {$_('knowledge.wizard.lockedConfigNotice.body', {
                default: 'Chunking strategy, embedding vendor / model, and vector DB are locked once the Knowledge Store is created. To change them, create a new Knowledge Store.'
            })}
        </p>
    </div>
    ...
    {:else if error}
        <div class="p-3 bg-red-50 border border-red-100 rounded text-sm text-red-700" role="alert">{error}</div>
    {:else}
        <div class="rounded-md border border-gray-200 bg-gray-50 p-3 text-sm text-gray-700 space-y-1">
            <div><span class="font-medium">{$_('knowledge.wizard.step6.chunkingLabel', { default: 'Chunking strategy' })}:</span> {chunkingStrategy || '-'}</div>
            <div><span class="font-medium">{$_('knowledge.wizard.step6.vendorLabel', { default: 'Embedding vendor' })}:</span> {embeddingVendor || '-'}</div>
            <div><span class="font-medium">{$_('knowledge.wizard.step6.modelLabel', { default: 'Embedding model' })}:</span> {embeddingModel || '-'}</div>
            <div><span class="font-medium">{$_('knowledge.wizard.step6.vectorDbLabel', { default: 'Vector DB' })}:</span> {vectorDb || '-'}</div>
        </div>
        ...
        {#if expanded}
            <div class="space-y-3 p-3 border border-gray-200 rounded-md">
                <div>
                ...
                <div>
                    <label for="wizard-ks-endpoint" class="block text-sm font-medium text-gray-700">
                        {$_('knowledge.wizard.step6.endpointLabel', { default: 'Embedding endpoint (optional)' })}
                    </label>
                    <input
                        type="text"
