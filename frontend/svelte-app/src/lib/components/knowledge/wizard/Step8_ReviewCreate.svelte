...
<div class="space-y-4">
    ...
    <p class="text-sm text-gray-600">
        {$_('knowledge.wizard.step8.description', {
            default: 'Double-check the summary below. Nothing has been created yet — clicking "Create" will create the resources and queue the ingestion.'
        })}
    </p>
    ...
    <div class="rounded-md border border-gray-200 divide-y">
        <div class="p-3">
            ...
            {#if wizardState.libraryDescription}
                <div class="text-xs text-gray-500 mt-1">{wizardState.libraryDescription}</div>
            {/if}
            {#if wizardState.libraryPath === 'new'}
        ...
        <div class="p-3">
            ...
            {#if wizardState.ksPath === 'new'}
                <div class="text-xs text-gray-500 mt-1 space-y-0.5">
                    <div>{$_('knowledge.wizard.step6.chunkingLabel', { default: 'Chunking strategy' })}: {wizardState.ksConfig?.chunking_strategy || '-'}</div>
                    <div>{$_('knowledge.wizard.step6.vendorLabel', { default: 'Embedding vendor' })}: {wizardState.ksConfig?.embedding_vendor || '-'}</div>
                    <div>{$_('knowledge.wizard.step6.modelLabel', { default: 'Embedding model' })}: {wizardState.ksConfig?.embedding_model || '-'}</div>
                    <div>{$_('knowledge.wizard.step6.vectorDbLabel', { default: 'Vector DB' })}: {wizardState.ksConfig?.vector_db_backend || '-'}</div>
                </div>
            {/if}
        </div>
    ...
    {#if submitting && progressMessage}
        <div class="rounded-md border border-blue-200 bg-blue-50 p-3 text-sm text-blue-800" aria-live="polite">
            {progressMessage}
        </div>
    {/if}
