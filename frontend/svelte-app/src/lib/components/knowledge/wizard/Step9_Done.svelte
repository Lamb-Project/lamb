...
<div class="space-y-4 text-center">
    ...
    <p class="text-sm text-gray-600">
        {$_('knowledge.wizard.step9.description', {
            default: 'Your Library and Knowledge Store are ready. Ingestion runs in the background — items will become queryable as they finish.'
        })}
    </p>

    <div class="rounded-md border border-gray-200 bg-gray-50 p-3 text-left text-sm space-y-1">
        {#if wizardState.createdRefs?.libraryName}
            <div>
                <span class="font-medium">{$_('knowledge.wizard.step8.libraryHeading', { default: 'Library' })}:</span>
                <span class="ml-1">{wizardState.createdRefs.libraryName}</span>
            </div>
        {/if}
        {#if wizardState.createdRefs?.ksName}
            <div>
                <span class="font-medium">{$_('knowledge.wizard.step8.ksHeading', { default: 'Knowledge Store' })}:</span>
    ...
    <div class="flex flex-wrap justify-center gap-2 pt-2">
        {#if wizardState.createdRefs?.ksId}
            <button
                type="button"
                onclick={openKS}
                class="px-4 py-2 text-sm font-medium text-white rounded-md shadow-sm bg-[#2271b3] hover:bg-[#195a91]"
            >
