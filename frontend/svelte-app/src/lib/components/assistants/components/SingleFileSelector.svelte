...
<div>
	...
	<div class="mb-4">
		<label for="file-upload" class="block text-sm text-gray-700"
			>{$_('assistants.form.singleFile.upload', { default: 'Upload New File' })}</label
		>
		<div class="mt-1 flex items-center">
			<input
				...
				class="block w-full text-sm text-gray-500 file:mr-4 file:rounded-md file:border-0 file:bg-gray-50 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-gray-700 hover:file:bg-gray-100 disabled:cursor-not-allowed"
				onchange={handleFileUpload}
				disabled={fileUploadLoading}
			/>
			{#if fileUploadLoading}
				<span class="ml-2 text-sm text-gray-500"
					>{$_('assistants.form.singleFile.uploading', { default: 'Uploading...' })}</span
				>
			{/if}
		</div>
		{#if fileUploadError}
	...
	{:else if error}
		<p class="text-sm text-red-600">
			{$_('assistants.form.singleFile.error', { default: 'Error loading files:' })}
			{error}
		</p>
	...
	{:else}
		<div class="mt-2 max-h-48 space-y-2 overflow-y-auto rounded border p-2">
			{#each userFiles as file (file.path)}
				<label
					class="flex cursor-pointer items-center space-x-2 p-1 {selectedFilePath === file.path
						? 'bg-blue-50'
						: 'hover:bg-gray-50'}"
				>
					<input
