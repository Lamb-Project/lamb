...
describe('validateImportedAssistant', () => {
	...
	test('reports missing required fields', () => {
		...
		expect(result.hasErrors).toBe(true);
		expect(result.validationLog.some((log) => log.startsWith('❌') && log.includes('name'))).toBe(
			true
		);
		expect(
			result.validationLog.some((log) => log.startsWith('❌') && log.includes('metadata'))
		).toBe(true);
	});
	...
	test('skips detailed checks when capabilities is null', () => {
		...
		expect(result.hasErrors).toBe(false);
		expect(result.validationLog.some((log) => log.includes('System capabilities not loaded'))).toBe(
			true
		);
	});
	...
	test('reports invalid prompt_processor', () => {
		const result = validateImportedAssistant(
			validAssistantJSON({
				metadata: JSON.stringify({
					prompt_processor: 'fake_processor',
					connector: 'openai',
					llm: 'gpt-4',
					rag_processor: 'no_rag'
				})
		...
		expect(
			result.validationLog.some((log) => log.startsWith('⚠️') && log.includes('prompt_processor'))
		).toBe(true);
	});
	...
	test('reports invalid connector', () => {
		...
		expect(
			result.validationLog.some((log) => log.startsWith('⚠️') && log.includes('connector'))
		).toBe(true);
	});
	...
	test('reports invalid LLM for valid connector', () => {
		...
		expect(result.validationLog.some((log) => log.startsWith('⚠️') && log.includes('llm'))).toBe(
			true
		);
	});
	...
	test('reports invalid rag_processor', () => {
		...
		expect(
			result.validationLog.some((log) => log.startsWith('⚠️') && log.includes('rag_processor'))
		).toBe(true);
	});
	...
	test('warns about RAG_Top_k when KB-based RAG used without it', () => {
		...
		expect(
			result.validationLog.some((log) => log.startsWith('⚠️') && log.includes('RAG_Top_k'))
		).toBe(true);
	});
	...
	test('warns about RAG_collections when KB-based RAG used without it', () => {
		...
		expect(
			result.validationLog.some((log) => log.startsWith('⚠️') && log.includes('RAG_collections'))
		).toBe(true);
	});
	...
	test('handles invalid metadata JSON', () => {
		...
		expect(
			result.validationLog.some((log) => log.startsWith('❌') && log.includes('metadata JSON'))
		).toBe(true);
		expect(result.hasErrors).toBe(true);
	});
