<script>
	...
	async function handleShare() {
		...
		try {
			...
			successMessage =
				$_('share_success', { values: { count: result.shared_count } }) ||
...
<div class="assistant-sharing-container p-4">
	{#if checkingPermission}
		<div class="py-4 text-center">
			...
			<p class="mt-2 text-sm text-gray-600">
				{$_('checking_permissions') || 'Checking permissions...'}
			</p>
		</div>
	{:else if !canShare}
	...
	{:else}
		...
		<div class="mb-6">
			<h3 class="mb-3 text-lg font-semibold">{$_('current_shares') || 'Current Shares'}</h3>
			...
			{:else if sharedUsers.length === 0}
				<p class="text-sm text-gray-500">
					{$_('no_shares_yet') || 'This assistant is not shared with anyone yet'}
				</p>
			{:else}
		...
		<div class="mb-6">
			<h3 class="mb-3 text-lg font-semibold">
				{$_('share_with_users') || 'Share with Organization Users'}
			</h3>
			{#if loadingUsers}
				<div class="flex justify-center py-4">
					<span class="loading loading-spinner loading-md"></span>
			...
			{:else if organizationUsers.length === 0}
				<p class="text-sm text-gray-500">
					{$_('no_users_to_share') || 'No other users in your organization'}
				</p>
			{:else}
						...
						<div class="form-control">
							<label class="label cursor-pointer justify-start gap-3">
								...
								<div class="flex-1">
									...
									<span class="label-text ml-2 text-gray-500">({user.email})</span>
									{#if user.user_type === 'end_user'}
										<span class="badge badge-sm badge-secondary ml-2"
											>{$_('end_user') || 'End User'}</span
										>
									{/if}
									{#if isAlreadyShared}
										<span class="badge badge-sm badge-success ml-2"
											>{$_('already_shared') || 'Already Shared'}</span
										>
									{/if}
								</div>
							</label>
				...
				{#if selectedUsers.length > 0}
					<div class="mt-4">
						<button class="btn btn-primary" onclick={handleShare} disabled={loading}>
							{#if loading}
								<span class="loading loading-spinner loading-sm"></span>
							{/if}
							{$_('share_with_selected', { values: { count: selectedUsers.length } }) ||
		...
		{#if assistant?.published}
			...
			<div class="mb-4">
				<button class="btn btn-outline btn-sm" onclick={toggleLtiUsers}>
					{showLtiUsers ? '▼' : '▶'}
					{$_('lti_users') || 'LTI Users'} ({ltiUsers.length})
				</button>
				...
				{#if showLtiUsers}
					<div class="mt-4">
						...
						{:else if ltiUsers.length === 0}
							<p class="text-sm text-gray-500">
								{$_('no_lti_users') || 'No LTI users have accessed this assistant yet'}
							</p>
						{:else}
							<div class="overflow-x-auto">
								<table class="table-zebra table w-full">
