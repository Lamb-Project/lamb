<script>
	...
	let currentTabKBs = $derived(currentTab === 'my' ? ownedKnowledgeBases : sharedKnowledgeBases);
	...
	async function handleDeleteConfirm() {
		...
		try {
			...
			successMessage = $_('knowledgeBases.list.deleteSuccess', {
				default: 'Knowledge base deleted.'
			});
			setTimeout(() => {
				successMessage = '';
			}, 4000);
	...
	async function handleToggleSharing(kb) {
		...
		try {
			...
			sharedKnowledgeBases = allKnowledgeBases.filter(
				(k) => k.is_owner === false && k.is_shared === true
			);
			...
			successMessage = newSharingState
				? $_('knowledgeBases.list.shareSuccess', {
						default: `"${kb.name}" is now shared with your organization.`
					})
...
<div class="overflow-hidden bg-white shadow sm:rounded-md">
	<div class="border-b border-gray-200 p-4 sm:flex sm:items-center sm:justify-between">
		...
		<div class="mt-3 sm:mt-0 sm:ml-4">
			<button
				...
				class="bg-brand hover:bg-brand-hover focus:ring-brand inline-flex items-center rounded-md border border-transparent px-4 py-2 text-sm font-medium text-white shadow-sm focus:ring-2 focus:ring-offset-2 focus:outline-none"
			>
	...
	{:else if error}
		<div class="p-6 text-center">
			...
			<button
				...
				class="bg-brand hover:bg-brand-hover focus:ring-brand mt-4 inline-flex items-center rounded-md border border-transparent px-4 py-2 text-sm font-medium text-white shadow-sm focus:ring-2 focus:ring-offset-2 focus:outline-none"
			>
	...
	{:else}
		...
		<div class="border-b border-gray-200">
			<div class="flex space-x-4 px-6 pt-4">
				...
				>
					...
					{#if currentTab === 'my'}
						<span class="bg-opacity-30 ml-2 rounded-full bg-white px-2 py-0.5 text-xs text-white">
				...
				<div class="flex">
					...
					<div class="ml-3">
						<p class="text-sm text-blue-700">
							{$_('knowledgeBases.list.sharedTabInfo', {
								default:
									'Note: Knowledge bases you own will not appear in this list, even if they are shared. Your own shared knowledge bases can be found in the "My Knowledge Bases" tab.'
							})}
						</p>
					</div>
				</div>
		...
		<!-- Filter Bar -->
		<FilterBar
			searchPlaceholder={$_('knowledgeBases.searchPlaceholder', {
				default: 'Search knowledge bases...'
			})}
		...
		<div class="mb-4 flex items-center justify-between px-6">
			<div class="text-sm text-gray-600">
				{#if searchTerm}
					...
					<span class="font-medium">{currentTabKBs.length}</span>
					{currentTab === 'my' ? 'owned' : 'shared'} knowledge bases
				{:else}
					<span class="font-medium">{totalItems}</span>
					{currentTab === 'my' ? 'owned' : 'shared'} knowledge {totalItems === 1 ? 'base' : 'bases'}
				{/if}
			</div>
		</div>
			...
			{#if currentTabKBs.length === 0}
				...
				<div class="p-6 text-center">
					<div class="text-gray-500">
						{currentTab === 'my'
							? $_('knowledgeBases.list.noOwned', {
									default: 'You have no knowledge bases yet. Create one to get started!'
								})
							: $_('knowledgeBases.list.noShared', {
									default: 'No shared knowledge bases available.'
								})}
					</div>
				</div>
			{:else}
				...
				<div class="p-6 text-center">
					<p class="mb-4 text-gray-500">No knowledge bases match your search</p>
					<button
						...
						{#each displayKnowledgeBases as kb (kb.id)}
							<tr>
								<td class="px-6 py-4 whitespace-nowrap">
									<div class="text-sm font-medium text-gray-900">
										<button
											...
											class="text-brand hover:text-brand-hover cursor-pointer border-0 bg-transparent p-0 text-left font-medium hover:underline"
										>
											{kb.name}
										</button>
									</div>
									...
									{#if kb.shared_by && !kb.is_owner}
										<div class="mt-1 text-xs text-gray-400">
											{$_('knowledgeBases.list.sharedBy', {
												values: { name: kb.shared_by },
												default: `Shared by ${kb.shared_by}`
											})}
										</div>
									{/if}
								</td>
								...
								<td class="px-6 py-4 whitespace-nowrap">
									{#if kb.is_owner}
										<span
											class="inline-flex rounded-full px-2 text-xs leading-5 font-semibold {kb.is_shared
												? 'bg-green-100 text-green-800'
												: 'bg-gray-100 text-gray-800'}"
										>
											{kb.is_shared
												? $_('knowledgeBases.list.shared', { default: 'Shared' })
												: $_('knowledgeBases.list.private', { default: 'Private' })}
										</span>
									{:else}
										<span
									...
									<div class="flex items-center justify-end gap-2">
										{#if kb.is_owner}
											...
											<button
												...
												class="rounded border px-2 py-1 text-sm {kb.is_shared
													? 'border-green-300 bg-green-50 text-green-700 hover:bg-green-100'
													: 'border-gray-300 bg-gray-50 text-gray-700 hover:bg-gray-100'}"
												title={kb.is_shared
													? $_('knowledgeBases.list.makePrivate', { default: 'Make private' })
													: $_('knowledgeBases.list.shareWithOrg', {
															default: 'Share with organization'
														})}
											>
...
<ConfirmationModal
	...
	message={$_('knowledgeBases.deleteModal.confirmation', {
