<script>
	...
	let visibleParamKeys = $derived.by(() => {
		...
		const paramKeys = Object.keys(selectedPlugin.parameters).filter(
	...
	$effect(() => {
		...
		if (
			...
			console.log(
				'[Job Polling] Starting auto-refresh for job',
				selectedJob.id,
				'status:',
				selectedJob.status
			);
	...
	async function loadIngestionJobs(id) {
		...
		try {
			const response = await listIngestionJobs(id, {
				limit: 200,
				sort_by: 'created_at',
				sort_order: 'desc'
			});
	...
	function getAcceptedFileTypes(plugin) {
		if (
			!plugin ||
			!Array.isArray(plugin.supported_file_types) ||
			plugin.supported_file_types.length === 0
		) {
			return '';
		}
	...
	async function runBaseIngestion() {
		console.log(
			'runBaseIngestion invoked for plugin:',
			selectedPlugin?.name,
			'params:',
			pluginParams
		);
	...
	function handleSubmitIngestion() {
		...
		if (
			...
			(!!selectedFile && pluginCanUseFileUpload(selectedPlugin))
		) {
			uploadFile();
		} else {
			runBaseIngestion();
						...
						<div>
							...
							<div class="mb-4 flex items-center justify-between">
								<div class="text-sm text-gray-500">
									{#if loadingJobs}
										<span class="inline-flex items-center">
											<svg
												class="mr-2 -ml-1 h-4 w-4 animate-spin text-[#2271b3]"
												fill="none"
												viewBox="0 0 24 24"
											>
												<circle
								...
								<button
									...
									class="inline-flex items-center rounded-md border border-gray-300 bg-white px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-50 focus:ring-2 focus:ring-[#2271b3] focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
								>
									<svg
										class="mr-1.5 h-4 w-4 {loadingJobs ? 'animate-spin' : ''}"
										fill="none"
										viewBox="0 0 24 24"
										stroke="currentColor"
									>
													...
													<td class="px-6 py-4 whitespace-nowrap">
														{#if job}
															<button
																...
																class="inline-flex cursor-pointer items-center gap-1 rounded-full px-2.5 py-1 text-xs font-medium ring-1 transition-opacity ring-inset hover:opacity-80 {statusColors.bg} {statusColors.text} {statusColors.ring}"
														...
														{:else}
															<span
																class="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-1 text-xs font-medium text-gray-500 ring-1 ring-gray-500/10 ring-inset"
															>
																Unknown
															</span>
														{/if}
									...
									>
											...
											<div>
												...
												>
													{selectedPlugin?.kind === 'file-ingest'
														...
														: $_('knowledgeBases.fileUpload.optionalFileLabel', {
																default: 'Optional File Input'
															})}
												</label>
												...
												{#if selectedPlugin?.kind !== 'file-ingest'}
													<p class="mt-2 text-xs text-gray-500">
														Optional: If a file is selected, ingestion runs through the file-upload
														path. Leave empty to run direct ingestion.
													</p>
												{/if}
												{#if selectedFile}
										...
										<div>
											<label
												...
												class="block text-sm font-medium text-gray-700"
											>
												{$_('knowledgeBases.fileUpload.pluginLabel', {
													default: 'Ingestion Plugin'
												})}
											</label>
											<div class="mt-1">
												<select
											...
											{#each Object.entries(selectedPlugin.parameters).filter( ([name]) => name.startsWith('_') ) as [paramName, paramDef] (paramName)}
												<div class="rounded-md border border-amber-200 bg-amber-50 p-4">
													<div class="flex items-start">
														<svg
															class="mt-0.5 mr-3 h-5 w-5 flex-shrink-0 text-amber-400"
															fill="currentColor"
															viewBox="0 0 20 20"
														>
															<path
													...
													{#if hasAdvancedParams}
														...
														>
															<span class="text-xs font-medium">
																{$_('knowledgeBases.fileUpload.advancedMode', {
																	default: 'Advanced'
																})}
															</span>
															<span
																class="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out {advancedMode
																	? 'bg-[#2271b3]'
																	: 'bg-gray-300'}"
																role="switch"
															...
															>
																<span
																	class="pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out {advancedMode
																		? 'translate-x-4'
																		: 'translate-x-0'}"
																></span>
															</span>
												...
												{#each displayedParamKeys as paramName (paramName)}
													...
													>
														...
														<label
															...
															class="block text-sm font-medium {hasVisibleWhenDependents(paramName)
																? 'text-[#2271b3]'
																: 'text-gray-700'}"
														>
															{paramName}
															...
															{#if hasVisibleWhenDependents(paramName)}
																<span
																	class="ml-2 rounded bg-blue-100 px-1.5 py-0.5 text-xs font-normal text-blue-700"
																>
																	controls options
																</span>
															{/if}
														...
														<div class="mt-1">
															{#if paramDef.enum && Array.isArray(paramDef.enum)}
																...
																<select
																	...
																	class="block w-full rounded-md border-gray-300 shadow-sm focus:border-[#2271b3] focus:ring-[#2271b3] sm:text-sm {hasVisibleWhenDependents(
																		paramName
																	)
																		? 'border-2 border-[#2271b3]'
																		: ''}"
										...
										<div
											class="flex flex-col gap-3 pt-4 sm:flex-row sm:items-center sm:justify-end"
										>
											...
											{/if}
											{#if uploadError}
												<div class="flex-1 rounded-md border border-red-200 bg-red-50 p-3">
											...
											<button
												type="submit"
												class="inline-flex flex-shrink-0 items-center rounded-md border border-transparent bg-[#2271b3] px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-[#195a91] focus:ring-2 focus:ring-[#2271b3] focus:ring-offset-2 focus:outline-none disabled:opacity-50"
												style="background-color: #2271b3;"
												disabled={!selectedPlugin ||
													uploading ||
													(selectedPlugin?.kind === 'file-ingest' && !selectedFile)}
											>
												{#if uploading}
...
{#if showJobModal && selectedJob}
	...
	>
		<div class="flex min-h-screen items-center justify-center px-4 pt-4 pb-20 text-center">
			...
			<!-- Modal panel -->
			<div
				class="relative mx-4 w-full max-w-2xl transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all"
			>
				<!-- Modal header -->
				<div class="border-b border-gray-200 bg-gray-50 px-4 py-4 sm:px-6">
					<div class="flex items-center justify-between">
						<div class="flex items-center gap-3">
							...
							<span
								class="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-medium ring-1 ring-inset {colors.bg} {colors.text} {colors.ring}"
							>
								{selectedJob.status}
							</span>
						</div>
						<button
							type="button"
							onclick={closeJobModal}
							class="rounded-md bg-white text-gray-400 hover:text-gray-500 focus:ring-2 focus:ring-[#2271b3] focus:outline-none"
						>
				...
				<!-- Modal body -->
				<div class="max-h-[60vh] overflow-y-auto px-4 py-5 sm:px-6">
					<!-- File info -->
					<div class="mb-6">
						<h4 class="mb-3 text-sm font-medium tracking-wider text-gray-500 uppercase">
							File Information
						</h4>
						<dl class="grid grid-cols-2 gap-x-4 gap-y-3">
							...
							<div>
								...
								<dd class="mt-0.5 text-sm text-gray-900">
									{selectedJob.file_size ? formatFileSize(selectedJob.file_size) : 'N/A'}
								</dd>
							</div>
							<div>
					...
					<div class="mb-6">
						...
						<dl class="grid grid-cols-2 gap-x-4 gap-y-3">
							...
							<div>
								...
								<dd class="mt-0.5 text-sm text-gray-900">
									{formatDuration(selectedJob.processing_duration_seconds)}
								</dd>
							</div>
							{#if selectedJob.processing_started_at}
								<div>
									...
									<dd class="mt-0.5 text-sm text-gray-900">
										{formatDate(selectedJob.processing_started_at)}
									</dd>
								</div>
							{/if}
							{#if selectedJob.processing_completed_at}
								<div>
									...
									<dd class="mt-0.5 text-sm text-gray-900">
										{formatDate(selectedJob.processing_completed_at)}
									</dd>
								</div>
							{/if}
						</dl>
					...
					{#if selectedJob.progress && (selectedJob.status === 'processing' || selectedJob.status === 'pending')}
						<div class="mb-6">
							...
							<div class="mb-2 h-2.5 w-full rounded-full bg-gray-200">
								<div
									class="h-2.5 rounded-full bg-[#2271b3] transition-all duration-300"
					...
					{#if selectedJob.processing_stats}
						...
						<div class="mb-6">
							<h4
								class="mb-3 flex items-center gap-2 text-sm font-medium tracking-wider text-gray-500 uppercase"
							>
								<svg
									class="h-4 w-4 text-[#2271b3]"
									fill="none"
									stroke="currentColor"
									viewBox="0 0 24 24"
								>
								...
								<dl class="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
									<div>
										...
										<dd class="font-medium text-blue-900">
											{formatNumber(stats.content_length)} characters
										</dd>
									</div>
									{#if stats.chunk_stats}
										<div>
											...
											<dd class="font-medium text-blue-900">
												{stats.chunk_stats.count} (avg {Math.round(stats.chunk_stats.avg_size)} chars)
											</dd>
										</div>
									{/if}
									...
									{#if stats.llm_calls && stats.llm_calls.length > 0}
										<div>
											...
											<dd class="font-medium text-blue-900">
												{stats.llm_calls.length} ({stats.images_with_llm_descriptions} successful)
											</dd>
										</div>
										<div>
											<dt class="text-xs text-blue-600">Total LLM Time</dt>
											<dd class="font-medium text-blue-900">
												{formatMilliseconds(stats.total_llm_duration_ms)}
											</dd>
										</div>
									{/if}
								</dl>
								...
								{#if stats.llm_calls && stats.llm_calls.length > 0}
									<details class="mt-3 text-xs">
										<summary class="cursor-pointer font-medium text-blue-700 hover:text-blue-900"
											>Show LLM call details ({stats.llm_calls.length} calls)</summary
										>
										<div class="mt-2 max-h-40 space-y-1.5 overflow-y-auto">
											{#each stats.llm_calls as call, i}
												<div
													class="flex items-center justify-between rounded bg-white/50 px-2 py-1 {call.success
														? ''
														: 'text-red-600'}"
												>
													<span class="flex-1 truncate font-mono">{call.image}</span>
													<span class="ml-2 whitespace-nowrap">
														{formatMilliseconds(call.duration_ms)}
														{#if call.tokens_used}
															<span class="ml-1 text-blue-500">({call.tokens_used} tokens)</span>
														{/if}
														{#if !call.success}
											...
											{#each stats.stage_timings as stage}
												<tr>
													<td class="px-3 py-2 font-medium text-gray-900 capitalize"
														>{stage.stage.replace(/_/g, ' ')}</td
													>
													<td class="px-3 py-2 font-mono text-gray-600"
														>{formatMilliseconds(stage.duration_ms)}</td
													>
								...
								<div class="flex flex-wrap gap-2">
									{#if stats.output_files.markdown_url}
										<a
											...
											target="_blank"
											rel="noopener noreferrer"
											class="inline-flex items-center gap-1.5 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-sm text-emerald-700 transition-colors hover:bg-emerald-100"
										>
									...
									{#if stats.output_files.images_folder_url}
										<a
											...
											target="_blank"
											rel="noopener noreferrer"
											class="inline-flex items-center gap-1.5 rounded-md border border-purple-200 bg-purple-50 px-3 py-1.5 text-sm text-purple-700 transition-colors hover:bg-purple-100"
										>
											<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
												<path
													stroke-linecap="round"
									...
									{#if stats.output_files.original_file_url}
										<a
											...
											target="_blank"
											rel="noopener noreferrer"
											class="inline-flex items-center gap-1.5 rounded-md border border-gray-200 bg-gray-50 px-3 py-1.5 text-sm text-gray-700 transition-colors hover:bg-gray-100"
										>
											<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
												<path
													stroke-linecap="round"
						...
						{#if stats.markdown_preview}
							<div class="mb-6">
								<details class="text-sm">
									<summary
										class="flex cursor-pointer items-center gap-2 font-medium text-gray-500 hover:text-gray-700"
									>
										<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
											<path
												stroke-linecap="round"
									...
									<pre
										class="mt-2 max-h-64 overflow-x-auto rounded-lg bg-gray-800 p-4 font-mono text-xs leading-relaxed whitespace-pre-wrap text-gray-100">{stats.markdown_preview}</pre>
								</details>
							</div>
						{/if}
					{/if}
					...
					{#if selectedJob.status === 'failed' && (selectedJob.error_message || selectedJob.error_details)}
						<div class="mb-6">
							<h4
								class="mb-3 flex items-center gap-2 text-sm font-medium tracking-wider text-red-600 uppercase"
							>
							...
							<div class="rounded-lg border border-red-200 bg-red-50 p-4">
								{#if selectedJob.error_message}
									<p class="mb-2 text-sm font-medium text-red-800">{selectedJob.error_message}</p>
								{/if}
								{#if selectedJob.error_details}
									<details class="text-xs">
										<summary class="cursor-pointer font-medium text-red-700 hover:text-red-900"
											>Show technical details</summary
										>
										<pre
											class="mt-2 overflow-x-auto rounded bg-red-100 p-3 text-xs whitespace-pre-wrap text-red-900">{JSON.stringify(
						...
						<div class="mb-4">
							<details class="text-sm">
								<summary class="cursor-pointer font-medium text-gray-500 hover:text-gray-700"
									>Plugin Parameters</summary
								>
								<pre
									class="mt-2 overflow-x-auto rounded bg-gray-50 p-3 text-xs whitespace-pre-wrap text-gray-700">{JSON.stringify(
										selectedJob.plugin_params,
										null,
										2
									)}</pre>
							</details>
				...
				<!-- Modal footer -->
				<div
					class="gap-2 border-t border-gray-200 bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6"
				>
					<!-- Refresh button -->
					<button
						type="button"
						...
						class="inline-flex w-full justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-base font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:ring-2 focus:ring-[#2271b3] focus:ring-offset-2 focus:outline-none disabled:opacity-50 sm:w-auto sm:text-sm"
						title="Refresh job status"
					>
						<svg
							class="h-4 w-4 {jobActionLoading ? 'animate-spin' : ''}"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
					...
					{#if selectedJob.status === 'failed' && kb?.can_modify === true}
						<button
							...
							disabled={jobActionLoading}
							class="inline-flex w-full justify-center rounded-md border border-transparent bg-[#2271b3] px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-[#195a91] focus:ring-2 focus:ring-[#2271b3] focus:ring-offset-2 focus:outline-none disabled:opacity-50 sm:ml-3 sm:w-auto sm:text-sm"
						>
							{#if jobActionLoading}
								<svg
					...
					{#if (selectedJob.status === 'pending' || selectedJob.status === 'processing') && kb?.can_modify === true}
						<button
							...
							disabled={jobActionLoading}
							class="inline-flex w-full justify-center rounded-md border border-red-300 bg-white px-4 py-2 text-base font-medium text-red-700 shadow-sm hover:bg-red-50 focus:ring-2 focus:ring-red-500 focus:ring-offset-2 focus:outline-none disabled:opacity-50 sm:ml-3 sm:w-auto sm:text-sm"
						>
							Cancel Job
						</button>
					{/if}
					<button
						type="button"
						onclick={closeJobModal}
						class="mt-3 inline-flex w-full justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-base font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:ring-2 focus:ring-[#2271b3] focus:ring-offset-2 focus:outline-none sm:mt-0 sm:w-auto sm:text-sm"
					>
						Close
					</button>
				</div>
...
<ConfirmationModal
	...
	message={$_('knowledgeBases.detail.confirmDeleteFile', {
		values: { filename: deleteFileTarget.filename },
		default: `Delete file "${deleteFileTarget.filename}" and all its embeddings? This action cannot be undone.`
	})}
...
<ConfirmationModal
	...
	message={$_('knowledgeBases.detail.confirmCancelJob', {
		default:
			'Are you sure you want to cancel this ingestion job? The job will be stopped and any progress will be lost.'
	})}
