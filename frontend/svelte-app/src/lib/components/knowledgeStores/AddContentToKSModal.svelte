    ...
    >
        ...
        >
            ...
            <div class="flex-1 overflow-y-auto px-6 py-4 space-y-4">
                {#if error}
                    <div class="p-3 bg-red-50 border border-red-100 rounded text-sm text-red-700" role="alert">
                        {error}
                    </div>
                {/if}
                ...
                {:else if libraries.length === 0}
                    <div class="text-sm text-gray-500">
                        {$_('knowledgeStores.addContentModal.noLibraries', {
                            default:
                                'No libraries available. Create a library and import some content first.',
                        })}
                    </div>
                {:else}
                    <div>
                    ...
                    <div>
                        ...
                        {:else}
                            <div class="mt-2 max-h-72 overflow-y-auto border border-gray-200 rounded">
                                {#each items as item (item.id)}
                                    <label
                                        class="flex items-center gap-3 px-3 py-2 border-b border-gray-100 hover:bg-gray-50 cursor-pointer"
                                    >
                                        <input
                                            type="checkbox"
            ...
            <div class="px-6 py-4 border-t border-gray-200 flex justify-end gap-2">
                <button
                    ...
                    onclick={close}
                    disabled={submitting}
                    class="px-4 py-2 text-sm font-medium text-gray-700 rounded-md border border-gray-300 hover:bg-gray-50 disabled:opacity-50"
                >
