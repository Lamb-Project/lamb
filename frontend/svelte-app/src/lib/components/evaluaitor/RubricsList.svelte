<script>
	...
	async function handleToggleVisibility(rubric) {
		try {
			...
			console.log(
				`Toggling rubric ${rubric.rubric_id} visibility to ${newVisibility ? 'public' : 'private'}`
			);
			...
			console.log(
				`Rubric visibility updated successfully to ${newVisibility ? 'public' : 'private'}`
			);
...
<div class="container mx-auto px-4 py-8">
	...
	{#if activeTab === 'templates'}
		<div class="mx-4 mb-4 rounded-md border border-blue-200 bg-blue-50 p-3">
			<div class="flex">
				...
				<div class="ml-3">
					<p class="text-sm text-blue-700">
						{localeLoaded
							? $_('rubrics.templatesTabInfo', {
									default:
										'Note: Rubrics you own will not appear in this list, even if they are public. Your own public rubrics can be found in the "My Rubrics" tab.'
								})
							: 'Note: Rubrics you own will not appear in this list, even if they are public. Your own public rubrics can be found in the "My Rubrics" tab.'}
					</p>
				</div>
	...
	{#if loading}
		<p class="py-4 text-center text-gray-500">
			{localeLoaded
				? $_('rubrics.loading', { default: 'Loading rubrics...' })
				: 'Loading rubrics...'}
		</p>
	{:else if error}
		<div
		...
		>
			<strong class="font-bold"
				>{localeLoaded ? $_('rubrics.errorTitle', { default: 'Error:' }) : 'Error:'}</strong
			>
	...
	{:else}
		...
		<FilterBar
			searchPlaceholder={localeLoaded
				? $_('rubrics.searchPlaceholder', { default: 'Search rubrics by title, description...' })
				: 'Search rubrics by title, description...'}
			...
			sortOptions={[
				{
					value: 'title',
					label: localeLoaded ? $_('common.sortByName', { default: 'Title' }) : 'Title'
				},
				{
					value: 'updated_at',
					label: localeLoaded
		...
		<div class="mb-4 flex items-center justify-between px-4">
			<div class="text-sm text-gray-600">
				{#if searchTerm}
					...
					<span class="font-medium">{allRubrics.length}</span> rubrics
				{:else}
			...
			{#if allRubrics.length === 0}
				...
				<div class="rounded-lg border border-gray-200 bg-white py-12 text-center">
					<p class="text-gray-500">
						{localeLoaded
							? $_('rubrics.noRubrics', { default: 'No rubrics found.' })
							: 'No rubrics found.'}
					</p>
				</div>
			{:else}
				...
				<div class="rounded-lg border border-gray-200 bg-white py-12 text-center">
					<p class="mb-4 text-gray-500">
						{localeLoaded
							? $_('rubrics.noMatches', { default: 'No rubrics match your search' })
				...
				<table class="min-w-full divide-y divide-gray-200">
					<thead class="bg-gray-50">
						<tr>
							...
							>
								{localeLoaded
									? $_('rubrics.table.name', { default: 'Rubric Name' })
									: 'Rubric Name'}
							</th>
							<th
								scope="col"
							...
							>
								{localeLoaded
									? $_('rubrics.table.description', { default: 'Description' })
									: 'Description'}
							</th>
							<th
								scope="col"
							...
							>
								{localeLoaded
									? $_('rubrics.table.gradeLevel', { default: 'Grade Level' })
									: 'Grade Level'}
							</th>
							<th
								scope="col"
							...
							<tr class="hover:bg-gray-50">
								...
								<td class="px-6 py-4 align-top whitespace-normal">
									...
									<div class="mt-1">
										{#if rubric.is_showcase}
											...
											>
												{localeLoaded
													? $_('rubrics.status.showcase', { default: 'Showcase' })
													: 'Showcase'}
											</span>
										{:else if rubric.is_public}
											...
											>
												{localeLoaded
													? $_('rubrics.status.public', { default: 'Public' })
													: 'Public'}
											</span>
										{:else}
											<span
												class="inline-flex rounded-full bg-gray-100 px-2 py-0.5 text-xs leading-5 font-semibold text-gray-800"
											>
												{localeLoaded
													? $_('rubrics.status.private', { default: 'Private' })
													: 'Private'}
											</span>
										{/if}
									</div>
								...
								<td class="px-6 py-4 align-top">
									<div class="max-w-md text-sm break-words text-gray-500">
										{rubric.description ||
											(localeLoaded
												? $_('rubrics.noDescription', { default: 'No description provided' })
								...
								<td class="px-6 py-4 align-top text-sm font-medium whitespace-nowrap">
									<div class="flex items-center space-x-1 sm:space-x-2">
										...
										<button
											onclick={() => handleView(rubric)}
											title={localeLoaded
												? $_('rubrics.actions.view', { default: 'View' })
												: 'View'}
										...
										{#if activeTab === 'my-rubrics'}
											...
											<button
												...
												title={localeLoaded
													? $_('rubrics.actions.edit', { default: 'Edit' })
													: 'Edit'}
											...
											<button
												...
												title={localeLoaded
													? $_('rubrics.actions.export', { default: 'Export (JSON & Markdown)' })
											...
											<button
												...
												title={rubric.is_public
													? localeLoaded
														? $_('rubrics.actions.makePrivate', { default: 'Make Private' })
														: 'Make Private'
													: localeLoaded
														? $_('rubrics.actions.makePublic', { default: 'Make Public' })
														: 'Make Public'}
											...
											<button
												...
												title={localeLoaded
													? $_('rubrics.actions.delete', { default: 'Delete' })
													: 'Delete'}
										...
										{:else}
											...
											<button
												...
												title={localeLoaded
													? $_('rubrics.actions.createCopy', { default: 'Create Copy' })
													: 'Create Copy'}
												class="rounded p-1 text-purple-600 transition-colors duration-150 hover:bg-purple-100 hover:text-purple-900"
											>
												{@html IconClone}
											</button>
										{/if}
									...
									<div class="mt-2 text-xs text-gray-400">
										{localeLoaded
											? $_('rubrics.lastModified', { default: 'Modified' })
											: 'Modified'}: {new Date(rubric.updated_at * 1000).toLocaleDateString()}
									</div>
