<script>
	...
	let criteria = $state([
		{
			...
			levels: [
				...
				},
				{
					...
					score: 3,
					label: 'Proficient',
					description: 'Demonstrates adequate understanding'
				},
				{
					id: 'level-1-3',
					score: 2,
					label: 'Developing',
					description: 'Demonstrates partial understanding'
				},
				{
					id: 'level-1-4',
					score: 1,
	...
	async function handleSubmit() {
		...
		try {
			...
			success = localeLoaded
				? $_('rubrics.form.successMessage', { default: 'Rubric saved successfully!' })
			...
			error =
				err.message ||
				(localeLoaded
					? $_('rubrics.form.errorMessage', { default: 'Failed to save rubric' })
	...
	function getMaxScoreHint(scoringType) {
		switch (scoringType) {
			case 'points':
				return localeLoaded
					? $_('rubrics.form.pointsHint', { default: 'Total possible points (e.g., 10, 20, 100)' })
					: 'Total possible points (e.g., 10, 20, 100)';
			case 'percentage':
				return localeLoaded
					? $_('rubrics.form.percentageHint', { default: 'Always 100 for percentage scoring' })
					: 'Always 100 for percentage scoring';
			case 'holistic':
				return localeLoaded
					? $_('rubrics.form.holisticHint', {
							default: 'Highest performance level (e.g., 4, 5, 6)'
						})
					: 'Highest performance level (e.g., 4, 5, 6)';
			case 'single-point':
				return localeLoaded
					? $_('rubrics.form.singlePointHint', { default: 'Number of criteria (typically 3-6)' })
					: 'Number of criteria (typically 3-6)';
			case 'checklist':
				return localeLoaded
					? $_('rubrics.form.checklistHint', { default: 'Number of checklist items' })
	...
	function getScoringTypeExplanation(scoringType) {
		switch (scoringType) {
			case 'points':
				return localeLoaded
					? $_('rubrics.form.pointsExplanation', {
							default: 'Analytic scoring with points for each criterion'
						})
					: 'Analytic scoring with points for each criterion';
			case 'percentage':
				return localeLoaded
					? $_('rubrics.form.percentageExplanation', {
							default: 'Scores expressed as percentages (0-100%)'
						})
					: 'Scores expressed as percentages (0-100%)';
			case 'holistic':
				return localeLoaded
					? $_('rubrics.form.holisticExplanation', {
							default: 'Single overall score (e.g., 1-4 scale)'
						})
					: 'Single overall score (e.g., 1-4 scale)';
			case 'single-point':
				return localeLoaded
					? $_('rubrics.form.singlePointExplanation', {
							default: 'Focus on meeting/not meeting expectations'
						})
					: 'Focus on meeting/not meeting expectations';
			case 'checklist':
				return localeLoaded
					? $_('rubrics.form.checklistExplanation', {
							default: 'Simple present/absent or yes/no format'
						})
					: 'Simple present/absent or yes/no format';
			default:
				return '';
		}
	}
	...
	$effect(() => {
		if (scoringType === 'percentage') {
			...
			if (maxScore === 100 || maxScore === 4) {
				// Reset from other types
				maxScore = 10;
			}
		}
	});
	...
	<div class="rounded-lg bg-white p-6 shadow">
		<div class="mb-6">
			<h2 class="text-2xl font-bold text-gray-900">
				{formState === 'create'
					? localeLoaded
						? $_('rubrics.form.createTitle', { default: 'Create New Rubric' })
						: 'Create New Rubric'
					: localeLoaded
						? $_('rubrics.form.editTitle', { default: 'Edit Rubric' })
						: 'Edit Rubric'}
			</h2>
			<p class="mt-1 text-sm text-gray-600">
				{localeLoaded
					? $_('rubrics.form.description', {
							default: 'Define your assessment criteria and performance levels'
						})
					: 'Define your assessment criteria and performance levels'}
			</p>
		</div>
		...
		<form onsubmit={handleSubmit}>
			...
			<div class="mb-6 grid grid-cols-1 gap-6 md:grid-cols-2">
				<div>
					<label for="title" class="block text-sm font-medium text-gray-700">
						{localeLoaded ? $_('rubrics.form.title', { default: 'Title' }) : 'Title'}
					...
					<input
						...
						placeholder={localeLoaded
							? $_('rubrics.form.titlePlaceholder', { default: 'e.g., Essay Writing Rubric' })
							: 'e.g., Essay Writing Rubric'}
					/>
				</div>
				...
				<div>
					...
					<input
						...
						class="mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
						placeholder={localeLoaded
							? $_('rubrics.form.subjectPlaceholder', {
									default: 'e.g., Mathematics, English, Science'
								})
			...
			<div class="mb-6 grid grid-cols-1 gap-6 md:grid-cols-2">
				<div>
					<label for="gradeLevel" class="block text-sm font-medium text-gray-700">
						{localeLoaded
							? $_('rubrics.form.gradeLevel', { default: 'Grade Level' })
							: 'Grade Level'}
					</label>
					<select
						id="gradeLevel"
					...
					>
						<option value="undefined"
							>{localeLoaded
								? $_('rubrics.form.gradeUndefined', { default: 'Undefined' })
								: 'Undefined'}</option
						>
				...
				<div>
					<label for="scoringType" class="block text-sm font-medium text-gray-700">
						{localeLoaded
							? $_('rubrics.form.scoringType', { default: 'Scoring Type' })
							: 'Scoring Type'}
					</label>
					<select
						id="scoringType"
			...
			<div class="mb-6 grid grid-cols-1 gap-6 md:grid-cols-2">
				<div>
					<label for="maxScore" class="block text-sm font-medium text-gray-700">
						{localeLoaded
							? $_('rubrics.form.maxScore', { default: 'Maximum Score' })
							: 'Maximum Score'}
					</label>
					<input
						type="number"
				...
				<div class="flex items-end">
					<div class="rounded border bg-gray-50 p-3 text-sm text-gray-600">
						<strong
							>{localeLoaded
								? $_('rubrics.form.scoringExplanation', { default: 'About Scoring Types:' })
								: 'About Scoring Types:'}</strong
						><br />
			...
			<div class="mb-6">
				<label for="description" class="block text-sm font-medium text-gray-700">
					{localeLoaded
						? $_('rubrics.form.description', { default: 'Description' })
						: 'Description'}
				</label>
				<textarea
					id="description"
					...
					class="mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
					placeholder={localeLoaded
						? $_('rubrics.form.descriptionPlaceholder', {
								default: 'Describe what this rubric assesses...'
							})
						: 'Describe what this rubric assesses...'}
				></textarea>
			</div>
			...
			<div class="mb-6">
				<div class="mb-4 flex items-center justify-between">
					<h3 class="text-lg font-medium text-gray-900">
						{localeLoaded
							? $_('rubrics.form.criteria', { default: 'Assessment Criteria' })
							: 'Assessment Criteria'}
					</h3>
					<button
						type="button"
						onclick={addCriterion}
						class="inline-flex items-center rounded border border-transparent bg-blue-100 px-3 py-1 text-sm font-medium text-blue-600 hover:bg-blue-200"
					>
						...
						{localeLoaded
							? $_('rubrics.form.addCriterion', { default: 'Add Criterion' })
							: 'Add Criterion'}
					</button>
				</div>
					...
					<div class="mb-4 rounded-md border border-gray-200 p-4">
						<div class="mb-3 flex items-start justify-between">
							<h4 class="text-md font-medium text-gray-900">
								{localeLoaded
									? $_('rubrics.form.criterion', { default: 'Criterion' })
									: 'Criterion'}
								{criterionIndex + 1}
							</h4>
							{#if criteria.length > 1}
						...
						<div class="mb-3 grid grid-cols-1 gap-4 md:grid-cols-3">
							<div class="md:col-span-2">
								<label
									for="criterion-name-{criterionIndex}"
								...
								<input
									...
									class="mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
									placeholder={localeLoaded
										? $_('rubrics.form.criterionNamePlaceholder', {
												default: 'e.g., Content Knowledge'
											})
							...
							<div>
								<label
									for="criterion-weight-{criterionIndex}"
									class="block text-sm font-medium text-gray-700"
								>
									{localeLoaded
										? $_('rubrics.form.weight', { default: 'Weight (%)' })
										: 'Weight (%)'}
								</label>
								<input
						...
						<div class="mb-3">
							<label
								for="criterion-description-{criterionIndex}"
								class="block text-sm font-medium text-gray-700"
							>
								{localeLoaded
									? $_('rubrics.form.criterionDescription', { default: 'Description' })
									: 'Description'}
							</label>
							<textarea
								...
								class="mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
								placeholder={localeLoaded
									? $_('rubrics.form.criterionDescriptionPlaceholder', {
											default: 'Describe what this criterion assesses...'
										})
									: 'Describe what this criterion assesses...'}
							></textarea>
						</div>
						...
						<div class="mb-3">
							<div class="mb-2 flex items-center justify-between">
								<h5 class="text-sm font-medium text-gray-700">
									{localeLoaded
										? $_('rubrics.form.performanceLevels', { default: 'Performance Levels' })
										: 'Performance Levels'}
								</h5>
								<button
									type="button"
									onclick={() => addLevel(criterionIndex)}
									class="inline-flex items-center rounded border border-transparent bg-blue-100 px-2 py-1 text-xs font-medium text-blue-600 hover:bg-blue-200"
								>
									<svg
										class="mr-1 -ml-1 h-3 w-3"
										fill="none"
										stroke="currentColor"
										viewBox="0 0 24 24"
									>
									...
									{localeLoaded
										? $_('rubrics.form.addLevel', { default: 'Add Level' })
										: 'Add Level'}
								</button>
							</div>
							...
							{#each criterion.levels as level, levelIndex}
								<div class="mb-3 flex items-start space-x-2">
									...
									<input
										...
										placeholder={localeLoaded
											? $_('rubrics.form.levelLabel', { default: 'Label' })
											: 'Label'}
									/>
									<textarea
										bind:value={level.description}
										rows="3"
										class="flex-1 rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500"
										placeholder={localeLoaded
											? $_('rubrics.form.levelDescription', { default: 'Description' })
											: 'Description'}
									></textarea>
									{#if criterion.levels.length > 2}
										<button
											...
											class="mt-1 p-1 text-red-600 hover:text-red-800"
											title={localeLoaded ? $_('common.remove', { default: 'Remove' }) : 'Remove'}
										>
			...
			<div class="flex justify-end space-x-3">
				...
				>
					...
					{:else}
						{formState === 'create'
							? localeLoaded
								? $_('common.create', { default: 'Create Rubric' })
								: 'Create Rubric'
							: localeLoaded
